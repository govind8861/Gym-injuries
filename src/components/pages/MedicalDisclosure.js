...
const MedicalDisclosure = () => {
	...
	useEffect(() => {
		...
		const welcomeTimer = setTimeout(() => {
		  setChatHistory(prevHistory => [
			...prevHistory,
			{ text: "Hello! Welcome to our injury chat web application. We currently cover the following 5 injuries:\n\n1. Rotator Cuff Injury\n\n2. Lower Back Pain\n\n3. Muscle Tear\n\n4. Knee Injury\n\n5. Spasms\n\nOur data is limited to these injuries, but we are actively working to expand our coverage with more injuries in a comprehensive way. Thank you for using our app!", isUser: false }
		]);
		}, delayToShowWelcome);
	...
	// 	if (bestMatch) {
	// 		return bestMatch.responses[0]; // Choose the first response with most matching tokens
	// 	} else {
	...
	const generateSystemResponse = userInput => {
		...
		for (const patternAndResponse of data) {
		  ...
		  for (const pattern of patterns) {
			...
			if (matchedTokens.length > exactMatchTokens && matchedTokens.length === patternTokens.length) {
